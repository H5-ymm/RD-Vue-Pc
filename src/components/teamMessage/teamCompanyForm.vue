...
<template>
  <div class="teamMessage">
    ...
    <div class="teamMessage-form-row">
      <el-form :model="companyForm" :rules="rules" ref="companyForm" label-width="100px" label-position="right" class="teamMessage-form">
        ...
        <el-form-item label="从事行业" prop="industry">
          <el-select v-model="companyForm.industry" value-key="label" class="width408" placeholder="请选择企业从事行业">
            <el-option :label="item.label" :value="item.value" v-for="(item,key) in jobList" :key="key"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="企业性质" required>
          <el-select v-model="companyForm.enterprise" value-key="label" class="width408" placeholder="请选择企业性质">
        ...
        <el-form-item label="团队简介" prop="introduction">
          <el-input type="textarea" class="width408" :autosize="{minRows: 5}" v-model="companyForm.introduction" placeholder="请输入团队介绍"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    updateTeamCompanyInfo() {
      ...
      updateTeamInfo(this.companyForm)
        .then(res => {
          if (res.data) {
